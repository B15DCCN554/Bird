...
<body>
	...
	<div class="wrap">
		<div class="container">
			<div
				style="display: flex; align-items: center; gap: 15px; background: url('/WebShop/style/assets/images/logoShop/bg-headtop.png') no-repeat center/cover; padding: 20px; border-radius: 8px;">
				...
				<div class="wrap-inner-table">
					<table>
						...
						<tbody>
							...
							<tr>
								<td><%=product.getId()%></td>
								<td style="max-width: 200px"><%=product.getFewChar()%></td>
								<td><%=Product.formMoney(product.getOriginalPrice())%></td>
								<td><%=Product.formMoney(product.getSalePrice())%></td>
								<td><img style="max-width: 70px"
									src="<%=product.getUrl()%>"></td>
								...
								<td>
									<div class="wrap-btn">
										<button type="button" class="btn-table btn-success--custom"
											...
											onclick="openEditModal('<%=product.getId()%>', '<%=product.getFewChar()%>', '<%=Product.formMoney(product.getOriginalPrice())%>'
											, '<%=Product.formMoney(product.getSalePrice())%>', '<%=product.getUrl()%>', '<%=product.getCategoryID()%>')">
					...
					<div class="modal-body">
						...
						<div class="mb-3">
							<label for="formFile" class="form-label">Ảnh</label> <input
								name="url" required class="form-control" type="text" required />
						</div>
						<div class="mb-3">
							<label for="recipient-name" class="col-form-label">Loại
								sản phẩm:</label> <select name="categoryID" class="form-select"
